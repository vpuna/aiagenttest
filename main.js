...
app.post("/users", async (req, res) => {
  try {
    ...
    // address is smallint, so validate as number and integer
    if (typeof address !== "number" || !Number.isInteger(address)) {
      return res.status(400).json({ error: "address is required (smallint)" });
    }

    const result = await pool.query(
      "INSERT INTO users (name, age, address) VALUES ($1, $2, $3) RETURNING *",
      [name.trim(), age, address]
    );
